...
All citations across this repository follow the IEEE numeric style as defined in [`CONTEXT.md` §6.4](../../CONTEXT.md). References are denoted by square-bracketed integers — e.g., `[1]`, or `[1], [3], [5]` for multiple sources, or `[1]–[4]` for a consecutive range. This file (`HeartSim-Report/05_references/README.md`) is the single master reference list for the entire repository; every other README uses the same numbering. When a new source is needed, it is appended here with the next available number — existing entries are never renumbered, so all cross-document citation numbers remain stable.
